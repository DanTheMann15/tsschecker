...
#define AUTH_VERSION "973.100.10"
#ifdef WIN32
...
int tss_request_add_local_policy_tags(plist_t request, plist_t parameters)
{
    ...
    if (_plist_dict_copy_item(request, parameters, "Ap,LocalPolicy", NULL) < 0) {
        tsserror("ERROR: Unable to find required Ap,LocalPolicy in parameters\n");
        return -1;
    }

    if (_plist_dict_copy_data(request, parameters, "Ap,NextStageIM4MHash", NULL) < 0) {
        tsserror("ERROR: Unable to find required Ap,NextStageIM4MHash in parameters\n");
        return -1;
    }
...
int tss_request_add_savage_tags(plist_t request, plist_t parameters, plist_t overrides, char **component_name)
{
    ...
    if (_plist_dict_copy_data(request, parameters, "Savage,UID", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,UID in parameters\n", __func__);
        return -1;
    }
    ...
    if (_plist_dict_copy_uint(request, parameters, "Savage,PatchEpoch", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,PatchEpoch in parameters\n", __func__);
        return -1;
    }

    if (_plist_dict_copy_uint(request, parameters, "Savage,ChipID", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,ChipID in parameters\n", __func__);
        return -1;
    }

    if (_plist_dict_copy_bool(request, parameters, "Savage,AllowOfflineBoot", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,AllowOfflineBoot in parameters\n", __func__);
        return -1;
    }

    if (_plist_dict_copy_bool(request, parameters, "Savage,ReadFWKey", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,ReadFWKey in parameters\n", __func__);
        return -1;
    }

    if (_plist_dict_copy_bool(request, parameters, "Savage,ProductionMode", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,ProductionMode in parameters\n", __func__);
        return -1;
    }
    ...
    if (_plist_dict_copy_data(request, parameters, "Savage,Nonce", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,Nonce in parameters\n", __func__);
        return -1;
    }

    if (_plist_dict_copy_bool(request, parameters, "Savage,ReadECKey", NULL) < 0) {
        tsserror("ERROR: %s: Unable to find required Savage,ReadECKey in parameters\n", __func__);
        return -1;
    }
...
int tss_request_add_yonkers_tags(plist_t request, plist_t parameters, plist_t overrides, char **component_name)
{
...

    {
        ...
        for (i = 0; i < (int)(sizeof(keys) / sizeof(keys[0])); ++i) {
            ...
            if (!node) {
                tsserror("ERROR: %s: Unable to find required %s in parameters\n", __func__, keys[i]);
            }
...
int tss_request_add_timer_tags(plist_t request, plist_t parameters, plist_t overrides)
{
    ...
    if (!node) {
        tsserror("ERROR: %s: Missing TicketName\n", __func__);
        return -1;
    }
    char key[64];
